...
class TestWindowSetup(unittest.TestCase):
    ...
    def test_window_setup_dark_mode_palette(self):
        ...
        try:
            ...
            assert tree_cfg.get('background') == '#0f172a'
            assert entry_cfg.get('fieldbackground') == '#0f172a'
            assert tab_cfg.get('background') == '#1e293b'
        finally:
            root.destroy()
...
class TestAutoConnectSetup(unittest.TestCase):
    ...
    @patch('src.gui.main_window.open_setup_wizard')
    @patch('src.gui.main_window.os.path.exists', return_value=True)
    def test_first_run_bootstrap_opens_setup_wizard(self, _mock_exists, mock_setup_wizard):
        """First-run bootstrap should schedule setup wizard automatically."""
        root = MagicMock()
        status_var = MagicMock()
        old_bootstrap = getattr(config, 'BOOTSTRAPPED_CONFIG', False)
        try:
            config.BOOTSTRAPPED_CONFIG = True
            setup_status_and_autoconnect(root, status_var, config_set=True)
            root.after.assert_any_call(100, unittest.mock.ANY)
            scheduled_callback = root.after.call_args_list[0][0][1]
            scheduled_callback()
            mock_setup_wizard.assert_called_once_with(root, status_var)
        finally:
            config.BOOTSTRAPPED_CONFIG = old_bootstrap
...
class TestDialogWindows(unittest.TestCase):
    ...
    def test_dialog_imports(self):
        ...
        assert callable(open_setup_wizard)
