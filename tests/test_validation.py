...
class TestEffectiveDownloadPath:
    ...
    def test_build_save_path_respects_prefix_setting_off(self):
        """Disabling season/year prefix setting should remove season-year path prefix generation."""
        old_prefix_pref = config.get_pref('prefix_imports', True)
        try:
            config.set_pref('prefix_imports', False)
            path = build_save_path(
                'Sample Prefix-Off Title',
                'Spring',
                '2026',
                'Anime - Web'
            )
            assert path == 'Sample Prefix-Off Title'
        finally:
            config.set_pref('prefix_imports', old_prefix_pref)
