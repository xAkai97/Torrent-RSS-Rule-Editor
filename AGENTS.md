...
- **GUI Framework:** PySide6 (Qt) as primary, with a legacy Tkinter fallback.
...
*   **`src/gui_qt/` & `src/gui/`**: Presentation layer. The primary PySide6 entry is `src/gui_qt/main_window.py`. Because the logic is decoupled into services, the Qt window acts mostly as a view that calls "service wrappers" (e.g., `run_qt_...` functions) to fetch/mutate data. Note that `main_window.py` is exceptionally large (>4000 lines) and complex.
...
*   **GUI State**: Handled locally via component properties or shared `AppState` classes, though the application tries to rely on `config.py` for persistent truth.
