...
from src.services.rules import RSSRule
from src.services import backup
from src.api.qbittorrent import fetch_rules, fetch_categories, fetch_feeds
...
def _show_import_sanitize_check(
    ...
    # Default to showing all titles in pre-import preview.
    show_changed_only_var = tk.BooleanVar(value=False)
    ...
    columns = ('severity', 'before', 'after', 'reason')
    ...
    table.heading('before', text='Display/Before Name')
    table.heading('after', text='After')
    table.heading('reason', text='Note')

    table.column('severity', width=85, minwidth=85, anchor='center', stretch=False)
    table.column('before', width=300, minwidth=200, anchor='w', stretch=True)
    table.column('after', width=250, minwidth=180, anchor='w', stretch=True)
    table.column('reason', width=250, minwidth=250, anchor='w', stretch=False)

    # Store original fixed column widths to prevent manual resizing
    fixed_col_widths = {'severity': 85, 'reason': 250}
    monitoring_id = [None]  # Use list to allow modification in nested function
    
    def _prevent_column_resize():
        """Reset fixed columns back to original widths if resized manually."""
        try:
            if not dlg.winfo_exists():
                return
            for col_name, target_width in fixed_col_widths.items():
                current_width = table.column(col_name, 'width')
                if current_width != target_width:
                    table.column(col_name, width=target_width)
            # Schedule next check
            monitoring_id[0] = dlg.after(200, _prevent_column_resize)
        except Exception:
            pass
    
    # Start monitoring for manual column resizes
    monitoring_id[0] = dlg.after(200, _prevent_column_resize)
    ...
    # Static column widths - no auto-fit needed for severity and reason (note)
    ...
    def _render_rows() -> None:
        ...
        for item in display_rows:
            ...
            # Combine before and display into single column for more space
            before_display = f"{item['before']}" if item['before'] == item['display'] else f"{item['display']} → {item['before']}"
            table.insert(
                '',
                'end',
                values=(sev_label, before_display, item['after'], item['reason']),
                tags=(severity,),
            )
    ...
    def _cancel() -> None:
        ...
        try:
            if monitoring_id[0]:
                dlg.after_cancel(monitoring_id[0])
            dlg.destroy()
        except Exception:
            pass
                ...
                try:
                    if isinstance(entry, dict):
                        ...
                        if orig_title and not orig_title.startswith(prefix):
                            ...
                            # Create season/year/title structure (qBit handles category path automatically)
                            import os.path
                            sanitized_title = sanitize_folder_name(orig_title)
                            # Save path is ONLY season/year/title, no category base
...
def populate_missing_rule_fields(
    ...
    Args:
        ...
        season: Season name
        year: Year string
        apply_default_save_path: If False, skip applying default save path (useful for season/year prefix)
    """
        ...
        for media_type, items in all_titles.items():
            ...
            for entry in items:
                ...
                try:
                    ...
                    # Apply default save path if missing (skip if prefix will create structure)
...
def _import_titles_core(
    ...
    try:
        ...
        if new_items:
            ...
            # Don't apply default save path if we're prefixing with season/year (cleanstructure)
...
def import_titles_from_file(
    ...
    if not path:
        path = filedialog.askopenfilename(
            title='Import titles file (JSON/CSV/TXT)',
                filetypes=[('JSON/CSV/TXT', '*.json *.csv *.txt'), ('JSON', '*.json'), ('CSV', '*.csv'), ('TXT', '*.txt'), ('All files', '*.*')]
        )
            ...
            try:
                ...
                if main_server_key != 'qbittorrent':
                    messagebox.showinfo(
                        ...
                        f'Direct sync is currently implemented for qBittorrent only.\n\n'
